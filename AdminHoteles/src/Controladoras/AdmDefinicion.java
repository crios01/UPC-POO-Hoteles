...
public class AdmDefinicion {
  ...
  public boolean verificaNumero(String numero) {
    ...
    if (log && Integer.parseInt(numero) > 0) {
      log = true;
    } else {
      log = false;
  ...
  public boolean escribirHabitacion(ArrayList<Habitacion> habitacion) {
    FileWriter fw = null;
    PrintWriter pw = null;
    boolean log = false;
    try {
  ...
  public ArrayList<Precio> listaPrecios(String correo) {
    ...
    FileReader fr = null;
    BufferedReader br = null;
    String linea, cadena1, cadena2, cadena3, cadena4, cadena5, cadena6, cadena7, cadena8, cadena9;
    int item, ini = 0, fin = 0;
    double precio5, precio7, precio9;
    try {
      ...
      while ((linea = br.readLine()) != null) {
        ini = 0;
        fin = linea.indexOf(",");
        cadena1 = linea.substring(ini, fin);    // Item
        item = Integer.parseInt(cadena1);
        ini = fin + 1;
        fin = linea.indexOf(",", ini);
        cadena2 = linea.substring(ini, fin);    // Correo
        ini = fin + 1;
        fin = linea.indexOf(",", ini);
        cadena3 = linea.substring(ini, fin);    // Tipo de Habitaciín
        ini = fin + 1;
        fin = linea.indexOf(",", ini);
        cadena4 = linea.substring(ini, fin);    // Moneda 1
        ini = fin + 1;
        fin = linea.indexOf(",", ini);
        cadena5 = linea.substring(ini, fin);    // Precio 1 - String
        precio5 = Double.parseDouble(cadena4);  // Precio 1 - Double
        ini = fin + 1;
        fin = linea.indexOf(",", ini);
        cadena6 = linea.substring(ini, fin);    // Moneda 2
        ini = fin + 1;
        fin = linea.indexOf(",", ini);
        cadena7 = linea.substring(ini, fin);    // Precio 2 - String
        precio7 = Double.parseDouble(cadena6);  // Precio 2 - Double
        ini = fin + 1;
        fin = linea.indexOf(",", ini);
        cadena8 = linea.substring(ini, fin);    // Moneda 3
        ini = fin + 1;
        fin = ini + 1;
        cadena9 = linea.substring(ini, fin);    // Precio 3 - String
        precio9 = Double.parseDouble(cadena8);  // Precio 3 - Double
        if (cadena1.equals(correo)) {
          precios.add(new Precio(item, cadena2, cadena3, cadena4, precio5, cadena6, precio7, cadena8, precio9));
        }
      }
    } catch (Exception e) {
      e.printStackTrace(); // Escribe la misma línea del texto de error, además del número exacto de línea de código.
  ...
  public boolean escribirPrecios(ArrayList<Precio> dbPrecios) {
    FileWriter fw = null;
    PrintWriter pw = null;
    boolean log = false;
    try {
      ...
      for (Precio precio : dbPrecios) {
        pw.print(String.valueOf(precio.getItem()) + ","
                + precio.getCorreo() + ","
                + precio.getTipHabitacion() + ","
  ...
  public ArrayList<TipoHabitacion> listaTipHabs() {
    ...
    BufferedReader br = null;
    try {
      fr = new FileReader(new File(miDir.getCanonicalPath() + "/tipoHabitaciones.txt"));
      br = new BufferedReader(fr);
      String linea;
      while ((linea = br.readLine()) != null) {
